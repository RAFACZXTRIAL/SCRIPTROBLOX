--[[ 
    ================================================
    MOUNT TARANJANG, JAPANESE & SIBUATAN AUTO SUMMIT - STELLAR SINGLE TAB
...
-- Fungsi ini TIDAK BERUBAH dan digunakan oleh semua mount.
...
local function startTaranjangLoop(count, delay)
    ...
    if runningSibuatan then stopSibuatanLoop() end -- TAMBAH: Hentikan Sibuatan
...
-- ===================================================================
local function startSibuatanLoop(count, delay)
    -- Jika loop lain sedang berjalan, hentikan dulu
    if runningTaranjang then stopTaranjangLoop() end
    if runningJapanese then stopJapaneseLoop() end
    ...
    StellarLibrary:Notify("Auto Summit Sibuatan Dimulai! Count: " .. (count == -1 and "Infinite" or count), 3);

    task.spawn(function()
        while runningSibuatan and (teleportsLeftSibuatan > 0 or teleportsLeftSibuatan == -1) do
            
            executeCycle(SIBUATAN_CFRAME, delayTimeSibuatan) -- Menggunakan CFRAME SIBUATAN
            
            if teleportsLeftSibuatan > 0 then
...
-- 4. KONTROL AUTO SUMMIT TARANJANG (TIDAK BERUBAH)
...
local TeleportCountTextboxSibuatan = SibuatanTab:Textbox("JUMLAH SUMMIT", "10", function(value)
    local count = tonumber(value)
    if count and count >= 0 then
        teleportsLeftSibuatan = (count == 0) and -1 or math.floor(count) 
    end
end)
